export default {
  async fetch(request, env) {
    ...
    // 2. Read raw body, log it, and parse JSON
    let bodyText = "";
    let update;
    try {
      bodyText = await request.text();
      console.log("Incoming Telegram Request Body:", bodyText);
      update = JSON.parse(bodyText);
    } catch (err) {
      console.error("Failed to parse JSON:", err);
      console.log("Attempted to parse body content:", bodyText);
    ...
    const TARGET_CHANNEL_ID_1 = -1003941932759;
    ...
    if (
     ...
     // msg.forward_from_chat && msg.forward_from_chat.id === TARGET_CHANNEL_ID_1
      ...
      if (match) {
        ...
        // OLD BINDING URL (Commented out)
        // const apiUrl = "https://hoviat-api-url.safaee1361.workers.dev/webhook/hoviat/v1/eurobot/set_group_message_id_test";
        
        // NEW DIRECT URL (Updated to .nip.io to bypass Error 1003)
        const apiUrl = "http://185.202.113.95.nip.io/webhook/hoviat/v1/eurobot/set_group_message_id_test";
    ...
    // const SENDER_ID_2 = -1003283701037;
    // const ORIGIN_ID_2 = -1003307384504;

    const SENDER_ID_2 = -1003997195070;
    const ORIGIN_ID_2 = -1003941932759;
    ...
    if (senderId === SENDER_ID_2 && originId === ORIGIN_ID_2) {
      ...
      // OLD BINDING URL (Commented out)
      // const apiUrl = "https://hoviat-api-url.safaee1361.workers.dev/webhook/hoviat/v1/eurobot/set_public_message_id_test";

      // NEW DIRECT URL (Updated to .nip.io to bypass Error 1003)
      const apiUrl = "http://185.202.113.95.nip.io/webhook/hoviat/v1/eurobot/set_public_message_id_test";
    ...
    // ==================================================================
    // SCENARIO 3: External Reply Check (Scenario 3 - New Log Values)
    // Sender: -1003872442653 ("Test هیلفن ستاره")
    // Origin: -1003941932759 ("Test امتیاز")
    // Target Endpoint: set_public_message_id
    // ==================================================================

    const SENDER_ID_3 = -1003872442653;
    const ORIGIN_ID_3 = -1003941932759;

    if (senderId === SENDER_ID_3 && originId === ORIGIN_ID_3) {
      console.log("Scenario 3 Met! External reply criteria matched.");

      // NEW DIRECT URL (Updated to .nip.io to bypass Error 1003)
      const apiUrl = "http://185.202.113.95.nip.io/webhook/hoviat/v1/hilfen/set_hilfen_message_id";
 ...
 * Modified to use standard fetch instead of binding, binding code preserved as comments.
 */
