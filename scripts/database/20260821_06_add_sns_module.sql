-- Migration 06.
-- Adds the SNS / User Panel schema: a bio column on users_eurobot, privacy
-- settings, social links, and bookmarks. Also wires `bio`/`bio_updated_at`
-- into the existing per-field timestamp triggers.
--
-- Run against the unified production database (after migrations 01-05).
...
-- ---------------------------------------------------------------------------
-- 1. Privacy visibility enum (public / private only)
-- ---------------------------------------------------------------------------
DO $BODY$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_type
        WHERE typname = 'privacy_scope' AND typtype = 'e'
    ) THEN
        CREATE TYPE privacy_scope AS ENUM ('public', 'private');
    END IF;
END;
$BODY$;

-- ---------------------------------------------------------------------------
-- 2. bio column on users_eurobot (must run before the trigger functions below,
--    because they reference the column via %ROWTYPE).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. User privacy settings (1-to-1 with users_eurobot)
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.user_privacy_settings (
    ...
    is_profile_discoverable     boolean NOT NULL DEFAULT true,

    profile_picture_visibility  privacy_scope NOT NULL DEFAULT 'public',
    username_visibility         privacy_scope NOT NULL DEFAULT 'public',
    first_name_visibility       privacy_scope NOT NULL DEFAULT 'public',
    last_name_visibility        privacy_scope NOT NULL DEFAULT 'public',
    nickname_visibility         privacy_scope NOT NULL DEFAULT 'public',
    country_visibility          privacy_scope NOT NULL DEFAULT 'public',

    phone_number_visibility     privacy_scope NOT NULL DEFAULT 'private',
    whatsapp_number_visibility  privacy_scope NOT NULL DEFAULT 'private',

    bio_visibility              privacy_scope NOT NULL DEFAULT 'public',
    social_links_visibility     privacy_scope NOT NULL DEFAULT 'public',

    updated_at                  timestamptz NOT NULL DEFAULT now()
);

-- ---------------------------------------------------------------------------
-- 4. User social media links (one-to-many per user)
-- ---------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_user_social_links_user_id
    ON public.user_social_links (user_id);

-- ---------------------------------------------------------------------------
-- 5. Bookmarks (many-to-many between users, with timestamp)
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.bookmarks (
    bookmarker_id      bigint NOT NULL
                       REFERENCES public.users_eurobot (user_id)
                       ON DELETE CASCADE,
    bookmarked_user_id bigint NOT NULL
                       ...
                       ON DELETE CASCADE,
    created_at         timestamptz NOT NULL DEFAULT now(),
    PRIMARY KEY (bookmarker_id, bookmarked_user_id)
);

CREATE INDEX IF NOT EXISTS idx_bookmarks_bookmarked_user_id
    ON public.bookmarks (bookmarked_user_id);

-- ---------------------------------------------------------------------------
-- 6. Re-declare the per-field timestamp trigger to include bio.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 7. Re-declare the updated_at trigger so bio_updated_at is ignored as a
--    "real change" when deciding whether to bump updated_at.
-- ---------------------------------------------------------------------------
...
COMMENT ON TABLE public.user_privacy_settings IS
    'Per-field privacy visibility for SNS user profiles';
COMMENT ON TABLE public.user_social_links IS
    'Social media / external links attached to SNS user profiles';
COMMENT ON TABLE public.bookmarks IS
    'Many-to-many bookmarks between SNS users';
